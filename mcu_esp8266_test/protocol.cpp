...
static unsigned char dp_download_colour_data_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    //debug
    // mcu_dp_value_update(DPID_COUNTDOWN,H);
    // mcu_dp_value_update(DPID_COUNTDOWN,S);
    // mcu_dp_value_update(DPID_COUNTDOWN,V);
    // mcu_dp_enum_update(DPID_WORK_MODE,value[0]);
    // mcu_dp_enum_update(DPID_WORK_MODE,value[1]);
    // mcu_dp_enum_update(DPID_WORK_MODE,value[2]);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
